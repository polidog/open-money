...
const modelCases: ModelCase[] = [
  {
    name: "共働き夫婦+子ども2人",
    description: "30代共働き夫婦。子ども2人、住宅ローンあり。典型的なファミリー世帯。",
...
export default function Home() {
  ...
  const inputCls = "w-full border-b border-neutral-300 bg-transparent px-0 py-3 text-base text-neutral-900 placeholder-neutral-400 focus:border-neutral-900 focus:outline-none transition-colors";
  const selectCls = "w-full border-b border-neutral-300 bg-transparent px-0 py-3 text-base text-neutral-900 focus:border-neutral-900 focus:outline-none transition-colors appearance-none cursor-pointer";

  const QuestionCard = ({ question, sub, children: content }: { question: string; sub?: string; children: React.ReactNode }) => (
    <div className="mx-auto w-full max-w-lg">
      <div className="py-8">
        <h2 className="mb-2 text-3xl font-medium tracking-tight text-neutral-900">{question}</h2>
  ...
  const NumberInput = ({ label, value, onChange, step: s, min, max, suffix }: {
    ...
    <div className="mb-6">
      <label className="mb-1 block text-xs font-medium uppercase tracking-widest text-neutral-500">{label}</label>
  ...
  const renderStep = () => {
    switch (step) {
        ...
        return (
          <QuestionCard question="あなたについて">
            <div className="grid grid-cols-2 gap-x-8 gap-y-2">
              <NumberInput label="年齢" value={person1.age} onChange={(v) => updatePerson1("age", v)} min={18} max={80} />
              ...
              <NumberInput label="年金月額（万円）" value={person1.pensionMonthly} onChange={(v) => updatePerson1("pensionMonthly", v)} step={1} min={0} suffix={fmtMan(person1.pensionMonthly)} />
            </div>
            <div className="mt-2">
              <p className="mb-3 text-xs font-medium uppercase tracking-widest text-neutral-500">配偶者</p>
              <YesNo value={hasPartner} onChange={setHasPartner} yesLabel="いる" noLabel="いない" />
            </div>
          </QuestionCard>
        );
      ...
      case 1:
        return (
          <QuestionCard question="配偶者について">
            <div className="grid grid-cols-2 gap-x-8 gap-y-2">
              <NumberInput label="年齢" value={person2.age} onChange={(v) => updatePerson2("age", v)} min={18} max={80} />
              ...
              <NumberInput label="年金月額（万円）" value={person2.pensionMonthly} onChange={(v) => updatePerson2("pensionMonthly", v)} step={1} min={0} suffix={fmtMan(person2.pensionMonthly)} />
            </div>
          </QuestionCard>
        );
      ...
      case 2:
        return (
          <QuestionCard question="世帯の生活">
            <NumberInput label="月間生活費（万円）" value={monthlyLiving} onChange={setMonthlyLiving} step={1} min={0} suffix={fmtMan(monthlyLiving)} />
      ...
      case 3:
        return (
          <QuestionCard question="お子さまについて">
            ...
            }} yesLabel="いる" noLabel="いない" />
            {hasChildren && children.length > 0 && (
              <div className="mt-8 space-y-6">
                {children.map((child, i) => (
                  <div key={`child-${i}-${child.name}`} className="border border-neutral-900 p-6">
                    <div className="mb-4 flex items-center justify-between">
                      ...
                      }} className="text-xs uppercase tracking-wider text-neutral-400 hover:text-neutral-900 transition-colors">削除</button>
                    </div>
                    <div className="mb-4">
                      <NumberInput label="年齢" value={child.age} onChange={(v) => updateChild(i, "age", v)} min={0} max={22} />
                    </div>
                    <p className="mb-3 text-xs font-medium uppercase tracking-widest text-neutral-500">進学プラン</p>
                    <div className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3 lg:grid-cols-5">
                      {schoolOptions.map((opt) => (
                        <div key={opt.key}>
                          <label className="mb-1 block text-xs text-neutral-400">{opt.label}</label>
                ...
                <button type="button" onClick={addChild} className="w-full border border-dashed border-neutral-300 py-3 text-xs uppercase tracking-wider text-neutral-400 hover:border-neutral-900 hover:text-neutral-900 transition-colors">
                  + もう1人追加
                </button>
              </div>
            )}
      ...
      case 4:
        return (
          <QuestionCard question="住宅ローン">
            <YesNo value={hasLoan} onChange={setHasLoan} yesLabel="あり" noLabel="なし" />
            {hasLoan && (
              <div className="mt-8">
                <div className="grid grid-cols-2 gap-x-8 gap-y-2">
                  <NumberInput label="借入額（万円）" value={loanAmount} onChange={setLoanAmount} step={100} min={0} suffix={fmtMan(loanAmount)} />
                  <NumberInput label="金利（%）" value={loanRate} onChange={setLoanRate} step={0.01} min={0} max={20} />
                  ...
                  <div className="mb-6">
                    <label className="mb-1 block text-xs font-medium uppercase tracking-widest text-neutral-500">返済方式</label>
                    <select value={loanMethod} onChange={(e) => setLoanMethod(e.target.value as RepaymentMethod)} className={selectCls}>
                ...
                <div className="mt-4">
                  <div className="mb-3 flex items-center justify-between">
                    <p className="text-xs font-medium uppercase tracking-widest text-neutral-500">繰り上げ返済</p>
                    <button type="button" onClick={addPrepayment} className="text-xs uppercase tracking-wider text-neutral-400 hover:text-neutral-900 transition-colors">+ 追加</button>
                  </div>
                  {loanPrepayments.map((p, i) => (
                    <div key={`pp-${i}-${p.yearFromStart}`} className="mb-3 flex flex-wrap items-end gap-6 border border-neutral-300 p-4">
                      <div>
                        <label className="mb-1 block text-xs text-neutral-400">年数</label>
                        <input type="number" value={p.yearFromStart} onChange={(e) => updatePrepayment(i, "yearFromStart", e.target.value)} className="w-20 border-b border-neutral-300 bg-transparent px-0 py-1.5 text-sm text-neutral-900 focus:border-neutral-900 focus:outline-none" min={1} />
                      </div>
                      <div>
                        <label className="mb-1 block text-xs text-neutral-400">金額（万円）</label>
                        <input type="number" value={p.amount} onChange={(e) => updatePrepayment(i, "amount", e.target.value)} className="w-32 border-b border-neutral-300 bg-transparent px-0 py-1.5 text-sm text-neutral-900 focus:border-neutral-900 focus:outline-none" min={0} step={10} />
                      </div>
                      <div>
                        <label className="mb-1 block text-xs text-neutral-400">タイプ</label>
                        <select value={p.type} onChange={(e) => updatePrepayment(i, "type", e.target.value)} className="border-b border-neutral-300 bg-transparent px-0 py-1.5 text-sm text-neutral-900 focus:border-neutral-900 focus:outline-none">
                          <option value="shorten">期間短縮型</option>
                          <option value="reduce">返済額軽減型</option>
                        </select>
                      </div>
                      <button type="button" onClick={() => removePrepayment(i)} className="text-xs text-neutral-400 hover:text-neutral-900 transition-colors pb-2">削除</button>
                    </div>
                  ))}
                </div>
              </div>
      ...
      case 6:
        return (
          <QuestionCard question="老後の生活費" sub="年金以外に必要な月々の生活費">
            <NumberInput label="月間生活費（万円）" value={retirementMonthlyExpense} onChange={setRetirementMonthlyExpense} step={1} min={0} suffix={fmtMan(retirementMonthlyExpense)} />
          </QuestionCard>
        );
  ...
  const tabs = [
    ...
    ...(result?.educationSummary.yearlyDetails.length ? [{ id: "education", label: "Education" }] : []),
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div className="mx-auto max-w-7xl px-6 pt-24 pb-20">
          {/* Hero */}
          <div className="mb-16">
            <p className="mb-3 text-xs font-medium uppercase tracking-[0.2em] text-neutral-400">Your Life Plan</p>
            <h2 className="text-5xl font-medium tracking-tight text-neutral-900 sm:text-6xl">シミュレーション結果</h2>
          </div>

          {/* Summary cards */}
          <div className="mb-16 grid grid-cols-2 gap-px bg-neutral-200 lg:grid-cols-4">
            <div className="bg-white p-8">
              <p className="mb-2 text-xs font-medium uppercase tracking-widest text-neutral-400">住宅ローン総額</p>
              <p className="text-2xl font-medium tracking-tight text-neutral-900">{fmtYenToMan(result.totalLoanPayment)}</p>
            </div>
            <div className="bg-white p-8">
              <p className="mb-2 text-xs font-medium uppercase tracking-widest text-neutral-400">教育費総額</p>
              <p className="text-2xl font-medium tracking-tight text-neutral-900">{fmtYenToMan(result.totalEducationCost)}</p>
            </div>
            <div className="bg-white p-8">
              <p className="mb-2 text-xs font-medium uppercase tracking-widest text-neutral-400">退職時の資産</p>
              <p className="text-2xl font-medium tracking-tight text-neutral-900">{fmtYenToMan(result.balanceAtRetirement)}</p>
            </div>
            <div className="bg-white p-8">
              <p className="mb-2 text-xs font-medium uppercase tracking-widest text-neutral-400">資産寿命</p>
              <p className={`text-2xl font-medium tracking-tight ${result.assetDepletionAge === null ? "text-neutral-900" : "text-red-600"}`}>
          ...
          {activeTab === "loan" && result.loanResult && (
            <div className="space-y-12">
              <div className="grid grid-cols-3 gap-px bg-neutral-200">
                <div className="bg-white p-8">
                  <p className="mb-2 text-xs font-medium uppercase tracking-widest text-neutral-400">{loanMethod === "equal_principal" ? "初回の支払額" : "月々の支払額"}</p>
                  <p className="text-2xl font-medium tracking-tight text-neutral-900">{fmtYen(result.loanResult.monthlyPayment)}</p>
                </div>
                <div className="bg-white p-8">
                  <p className="mb-2 text-xs font-medium uppercase tracking-widest text-neutral-400">総支払額</p>
                  <p className="text-2xl font-medium tracking-tight text-neutral-900">{fmtYen(result.loanResult.totalPayment)}</p>
                </div>
                <div className="bg-white p-8">
                  <p className="mb-2 text-xs font-medium uppercase tracking-widest text-neutral-400">利息合計</p>
                  <p className="text-2xl font-medium tracking-tight text-neutral-900">{fmtYen(result.loanResult.totalInterest)}</p>
                </div>
              </div>
          ...
          {activeTab === "education" && result.educationSummary.yearlyDetails.length > 0 && (
            <div>
              <div className="mb-8">
                <h3 className="mb-6 text-2xl font-medium tracking-tight text-neutral-900">教育費の詳細</h3>
                  ...
                  <div className="mb-8 flex flex-wrap gap-px bg-neutral-200 w-fit">
                    {result.educationSummary.perChildTotal.map((child) => (
                      <div key={child.name} className="bg-white px-8 py-5">
                        <p className="mb-1 text-xs font-medium uppercase tracking-widest text-neutral-400">{child.name}</p>
                        <p className="text-lg font-medium tracking-tight text-neutral-900">{fmtYenToMan(child.total)}</p>
                      </div>
                    ))}
                    {result.educationSummary.perChildTotal.length > 1 && (
                      <div className="bg-neutral-50 px-8 py-5">
                        <p className="mb-1 text-xs font-medium uppercase tracking-widest text-neutral-400">合計</p>
                        <p className="text-lg font-medium tracking-tight text-neutral-900">{fmtYenToMan(result.educationSummary.grandTotal)}</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-neutral-900 text-left">
                      ...
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400">子ども</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400">年齢</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400">学校</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400">種別</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400 text-right">年間費用</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400 text-right">年合計</th>
                    </tr>
                  </thead>
                  <tbody>
                    {result.educationSummary.yearlyDetails.map((year) =>
                      year.children.map((child, ci) => (
                        <tr key={`${year.parentAge}-${child.childName}`} className="border-b border-neutral-100 hover:bg-neutral-50 transition-colors">
                          {ci === 0 && <td className="px-4 py-3 font-medium text-neutral-900" rowSpan={year.children.length}>{year.parentAge}歳</td>}
                          <td className="px-4 py-3 text-neutral-600">{child.childName}</td>
                          <td className="px-4 py-3 text-neutral-600">{child.childAge}歳</td>
                          <td className="px-4 py-3 text-neutral-600">{child.stage}</td>
                          <td className="px-4 py-3 text-neutral-600">{child.schoolType}</td>
                          <td className="px-4 py-3 text-right text-neutral-600">{fmtYenToMan(child.cost)}</td>
                          {ci === 0 && <td className="px-4 py-3 text-right font-medium text-neutral-900" rowSpan={year.children.length}>{fmtYenToMan(year.totalCost)}</td>}
                        </tr>
                      )),
                    )}
                  </tbody>
          ...
          {activeTab === "table" && (
            <div>
              <h3 className="mb-2 text-2xl font-medium tracking-tight text-neutral-900">資産推移テーブル</h3>
              <p className="mb-8 text-xs uppercase tracking-widest text-neutral-400">Asset Transition Table</p>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-neutral-900 text-left">
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400">年齢</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400">区分</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400 text-right">年収入</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400 text-right">生活費</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400 text-right">ローン</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400 text-right">教育費</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400 text-right">年間収支</th>
                      <th className="px-4 py-3 text-xs font-medium uppercase tracking-widest text-neutral-400 text-right">資産残高</th>
                    </tr>
                  </thead>
                  <tbody>
                    {result.schedule.map((row) => (
                      <tr key={row.age} className={`border-b border-neutral-100 hover:bg-neutral-50 transition-colors ${row.age === person1.retirementAge ? "bg-neutral-50" : ""} ${row.balance < 0 ? "bg-red-50/50" : ""}`}>
                        <td className="px-4 py-3 font-medium text-neutral-900">{row.age}歳</td>
                        <td className="px-4 py-3 text-neutral-500">{row.phase}</td>
                        <td className="px-4 py-3 text-right text-neutral-600">{fmtYenToMan(row.income)}</td>
                        <td className="px-4 py-3 text-right text-neutral-600">{fmtYenToMan(row.livingCost)}</td>
                        <td className="px-4 py-3 text-right text-neutral-600">{row.loanPayment > 0 ? fmtYenToMan(row.loanPayment) : "-"}</td>
                        <td className="px-4 py-3 text-right text-neutral-600">{row.educationCost > 0 ? fmtYenToMan(row.educationCost) : "-"}</td>
                        <td className={`px-4 py-3 text-right font-medium ${row.savings >= 0 ? "text-neutral-900" : "text-red-600"}`}>
                          {row.savings >= 0 ? "+" : ""}{fmtYenToMan(row.savings)}
                        </td>
                        <td className={`px-4 py-3 text-right font-medium ${row.balance >= 0 ? "text-neutral-900" : "text-red-600"}`}>
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <div className="grid grid-cols-1 gap-px bg-neutral-200 sm:grid-cols-2 lg:grid-cols-3">
              {modelCases.map((mc) => (
                ...
                >
                  <h3 className="mb-3 text-lg font-medium tracking-tight text-neutral-900 group-hover:underline underline-offset-4">{mc.name}</h3>
                  <p className="text-sm leading-relaxed text-neutral-500">{mc.description}</p>
                </button>
              ))}
            </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 pt-20 pb-8">
        ...
        <div className="mx-auto mt-12 w-full max-w-lg">
          <div className="mb-4 flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-neutral-400 shrink-0">{step + 1} / {TOTAL_STEPS}</span>
          </div>
